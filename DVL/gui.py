...
import uml_creator as uml_creator
import sys
import os
...
# Erstellen Sie ein Notebook für die Hauptregisterkarten
main_notebook = ttk.Notebook(main_frame)
main_notebook.grid(row=0, column=0, rowspan=3, sticky="nsew")

# SQL-Worksheet-Tab
sql_tab = tk.Frame(main_notebook, bg=BG)
main_notebook.add(sql_tab, text=f"SQL Worksheet")

# UML Diagram-Tab
uml_tab = tk.Frame(main_notebook, bg=BG)
main_notebook.add(uml_tab, text=f"{UML_ICON} UML Diagram")

# SQL Worksheet-Komponenten
sql_header = tk.Frame(sql_tab, height=30, bg=HEADER_BG, relief="flat")
...
entry_frame = tk.Frame(sql_tab, bg=BG)
entry_frame.pack(fill="both", expand=False, padx=5, pady=5)
...
button_frame = tk.Frame(sql_tab, bg=BG)
button_frame.pack(fill="x", padx=10, pady=(0, 5), anchor="e")
...
response_frame = tk.Frame(sql_tab, bg=BG)
response_frame.pack(fill="x")
...
results_notebook = ttk.Notebook(sql_tab)
results_notebook.pack(fill="both", expand=True, padx=5, pady=(0, 5))
...
# UML-Diagramm-Komponenten initialisieren
uml_creator.init_uml_tab(uml_tab, BG, TEXT_BG, TEXT_FG, ACCENT, ACCENT_HOVER, HEADER_BG, FONT, FONT_MONO)
...
def refresh_sidebar():
    ...
    for db in os.listdir("C:/LiteDBox"):
        if os.path.isdir(os.path.join("C:/LiteDBox", db)) and db != "Diagrams":
...
# Stellen Sie sicher, dass der Diagrams-Ordner existiert
diagrams_dir = os.path.join("C:/LiteDBox", "Diagrams")
